<template>
  <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-3 sm:p-3.5 shadow-xl hover:shadow-2xl transition-all duration-300 flex flex-col justify-between h-full group" id="product-card">
    <div>
      <!-- Top Image Frame -->
      <div class="relative aspect-square rounded-2xl overflow-hidden bg-slate-50 dark:bg-slate-800 mb-3 group/img">
        ...
        <span v-else class="absolute top-2.5 left-2.5 z-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md text-slate-800 dark:text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-sm">
          Best Seller
        </span>

        <!-- Top Right Brand/Logo Circle Badge -->
        <div class="absolute top-2.5 right-2.5 z-10 w-7 h-7 rounded-full bg-white dark:bg-slate-900 shadow-md flex items-center justify-center text-brand-500 font-black text-xs border border-slate-100 dark:border-slate-800">
          B
        </div>
      ...
      <div class="px-1">
        <!-- Product Name -->
        <router-link :to="{ name: 'product-detail', params: { id: product.id } }" class="block text-sm sm:text-base font-black text-slate-900 dark:text-white leading-snug line-clamp-1 hover:text-brand-500 transition-colors mb-0.5 tracking-tight">
          {{ product.name }}
        </router-link>

        <!-- Subtitle / Category -->
        <span class="text-[11px] font-semibold text-slate-400 block mb-1">
          {{ product.category?.name || 'Own the BeSmart Edition' }}
        </span>

        <!-- Description Snippet -->
        <p class="text-[11px] text-slate-400 dark:text-slate-400 line-clamp-2 leading-relaxed mb-3 min-h-[2.5em]">
    ...
    <div class="mt-auto pt-2 border-t border-slate-100 dark:border-slate-800/80 flex flex-col gap-2 w-full">
      <!-- Price & Discount Row -->
      <div class="flex items-center justify-between px-1">
        <span class="text-[11px] text-slate-400 font-medium">Price</span>
        <div class="flex items-baseline gap-1.5">
          <span class="text-sm sm:text-base font-black text-slate-900 dark:text-white">৳{{ (product.sale_price || product.price).toLocaleString() }}</span>
          <span v-if="product.sale_price && product.sale_price < product.price" class="text-[10px] text-slate-400 line-through">৳{{ product.price.toLocaleString() }}</span>
        </div>
      </div>
      ...
      <button
        ...
        class="w-full py-2 sm:py-2.5 px-3 rounded-full bg-brand-500 hover:bg-brand-600 active:scale-95 text-white font-extrabold text-xs flex items-center justify-center gap-1.5 shadow-md hover:shadow-lg transition-all duration-200 cursor-pointer"
        id="add-to-cart-btn"
      >
        <span>Buy Now</span>
